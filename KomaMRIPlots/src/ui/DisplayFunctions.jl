...
# TODO Adapt the documentation

"""
    p = plot_state_map(obj::Phantom, key::Symbol; kwargs...)
Plots a phantom map for a specific spin parameter given by `key`.
# Arguments
- `obj`: (`::Phantom`) Phantom struct
- `key`: (`::Symbol`, opts: [`:ρ`, `:T1`, `:T2`, `:T2s`, `:x`, `:y`, `:z`]) symbol for
...
function plot_state_map(
    ...
    path = @__DIR__
    if key == :Mxy || key == :Mz || key == :∠Mxy || key == :RealOfMxy || key ==:ImagOfMxy
        cmin_key = 0
        factor = 1
        unit = " a.u."
        colormap = "Greys"
        if key == :Mxy
            cmax_key = maximum(abs.(ph.Mxy))
            cmin_key = minimum(abs.(ph.Mxy))
            x, y, z = ph.x, ph.y, abs.(ph.Mxy)
        elseif key == :Mz
            cmax_key = maximum(abs.(ph.Mz))
            cmin_key = minimum(abs.(ph.Mz))
            x, y, z = ph.x, ph.y, abs.(ph.Mz)
        elseif key == :∠Mxy 
            cmax_key = maximum(angle.(ph.Mxy))
            cmin_key = minimum(angle.(ph.Mxy))
            x, y, z = ph.x, ph.y, angle.(ph.Mxy)
        elseif key == :RealOfMxy
            cmax_key = maximum(real.(ph.Mxy))
            cmin_key = minimum(real.(ph.Mxy))
            x, y, z = ph.x, ph.y, real.(ph.Mxy)
        elseif key == :ImagOfMxy
            cmax_key = maximum(imag.(ph.Mxy))
            cmin_key = minimum(imag.(ph.Mxy))
            x, y, z = ph.x, ph.y, imag.(ph.Mxy)
        end
    else
        factor = 1        
